function vect = przeanalizuj5(V,instr)
            ...
            if sum(V == 1) == sum(instr)
                ...
                for i = 1:size(MATRI,1)
                    ...
                    for j = 1:length(spaces)
                        if spaces(j) > 0
                            needs(j) = sums(j) + spaces(j);
                        else
                            needs(j) = sums(j);
                        end
                    end
                    %sums,spaces,needs,onez,
                    if (sum(needs <= lens) == length(lens)) && (sum(onez <= sums) == length(onez))
                        VALID = [VALID; p];
                    end
                end
                VALID;
                ...
                for i = 1:N_groups
                    ...
                    for j = 1:length(SE)
                        ...
                        if ~isempty(segment)
                            ...
                            n = length(segment) + 1;
                            k = len - sum(segment) - length(segment) + 1;%len, segment,VALID,
